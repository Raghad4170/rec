...
class custody_perennial(models.Model):
    _name = 'custody.perennial' 
    _description = 'العهدة المالية المستديمة'
    _inherit = ['portal.mixin', 'mail.thread', 'mail.activity.mixin', 'utm.mixin']

    name = fields.Char(string="العهدة المالية المستديمة", tracking=True)
    user_id = fields.Many2one(related='employee_id.user_id', tracking=True)
    employee_id = fields.Many2one('hr.employee', string='الموظف', required=True, default=lambda self: self.env.user.employee_id, tracking=True)
    bills_ids = fields.One2many('bills.management', 'custody_id', string='الفواتير', tracking=True)
    company_id = fields.Many2one('res.company', 'الشركة', index=True, default=lambda self: self.env.company, tracking=True)
    ...
    sign = fields.Binary(string="التوقيع", tracking=True)
    custody = fields.Float(string="المبلغ", tracking=True)
    custody_total = fields.Float(string="مبلغ إجمالي الفواتير المعتمدة", tracking=True, compute="_get_custody_remaining")
    custody_amount = fields.Float(string="المتبقي من مبلغ العهدة", tracking=True, compute="_get_custody_remaining")
    custody_paid = fields.Float(string="المبلغ المسدد", tracking=True, compute="_get_custody_remaining")
    custody_paid_total = fields.Float(string="إجمالي قيمة الفواتير المسددة", tracking=True, compute="_get_custody_remaining")
    iban = fields.Char(string="رقم حساب (الآيبان)", tracking=True)
    back = fields.Date(string='تاريخ اخلاء الطرف', tracking=True)
    state = fields.Selection([
        ('draft','مسودة'),
    ...
    def _get_custody_remaining(self):
        for custody in self:
            ...
            custody.custody_total = custody_total
            custody_amount = custody_remaining - custody_total
            custody.custody_amount = custody_amount
#             custody_paid_total = custody_paid_total
            custody.custody_paid_total = custody_paid_total 
            custody_paid = custody_amount + custody_paid_total
            custody.custody_paid = custody_paid
